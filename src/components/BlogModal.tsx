import {DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle} from "./ui/dialog.tsx";
...
const BlogModal: React.FC<Blog> = ({title, description, imgUrl, createdAt, user, likeCount, comments}) => {
    return (
        <DialogContent className="h-[90vh] flex flex-col justify-start">
            <DialogHeader>
                <DialogTitle>{title}</DialogTitle>
                <DialogDescription className="text-xs">{user.name} - {createdAt}</DialogDescription>
            </DialogHeader>
            ...
            <div className="absolute left-4 bottom-4 right-4 bg-white">
                <div className="flex">
                    <Input className="px-3 py-6 rounded-tr-none rounded-br-none"
                           placeholder="Các hạ xin hãy viết bình luận tại đây"/>
                    <Button variant="outline"
                            className="py-6 rounded-tl-none rounded-bl-none ml-[-1px]">Gửi</Button>
                </div>
            </div>
        </DialogContent>
    );
